...
final class CreateCompiler {
    ...
    List<CreatePlan> compileCreates(
        ...
        for (int i = 0; i < rule.create.size(); i++) {
            ...
            if (targetClass == null) {
                ctx.diagnostics()
                        .add(new Diagnostic(
                                DiagnosticCode.MAP_CREATE_UNKNOWN_CLASS,
                                Severity.ERROR,
                                "Create target class not found in model: " + targetClassName,
                                createId,
                                "Check the class name and model registration"));
                continue;
            }
            ...
            if (cs.assign != null) {
                for (var entry : cs.assign.entrySet()) {
                    ...
                    CompiledExpression expr =
                            ctx.expressionCompiler().compile(entry.getValue(), exprCtx, ctx.diagnostics());
                    if (expr != null) {
